...
import { websiteBannersApi } from "@/lib/api-client";
...
export default function WebsiteBannersPage() {
  ...
  const fetchBanners = async () => {
    try {
      setIsLoading(true);
      const data = await websiteBannersApi.getAll();
      setBanners(data);
    } catch (error) {
      console.error("Failed to fetch website banners:", error);
      alert("Failed to load website banners");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleImageUpload = async (files: FileList | null) => {
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        await websiteBannersApi.create(formData);
      }
      await fetchBanners();
    } catch (error) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await websiteBannersApi.delete(deletingBanner.id);
      await fetchBanners();
